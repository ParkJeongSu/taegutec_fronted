    ...
    <v-card class="elevation-1 rounded-lg pa-4">
      ...
      <div class="d-flex flex-wrap align-center justify-space-between mb-4">
        <div class="d-flex align-center mb-2 mb-sm-0">
          ...
          <span class="text-h6 font-weight-bold text-high-emphasis">POWDER SAP Idoc 인터페이스 이력</span>
          <v-chip size="small" color="secondary" variant="tonal" class="ml-3 font-weight-medium">
            오더 &gt; 인터페이스 &gt; POWDER Idoc 이력
          </v-chip>
      ...
      >
        ...
        <template #[`item.docNum`]="{ item }">
          <span class="font-weight-bold text-secondary cursor-pointer">{{ item.docNum }}</span>
        </template>

        <!-- IDOC 타입 ID 칩 (타입 19인 경우 Part 뱃지 추가) -->
        <template #[`item.idocTypId`]="{ item }">
          <div class="d-flex align-center justify-center">
            <v-chip size="x-small" color="secondary" variant="tonal" class="font-weight-bold mr-1">
              {{ item.idocTypId || item.idocType || '-' }}
            </v-chip>
            <v-chip
              v-if="String(item.idocTypId || item.idocType) === '19'"
              size="x-small"
              color="indigo"
              variant="flat"
        ...
        <template #no-data>
          <div class="text-center py-6 text-medium-emphasis">
            ...
            <div>조회된 POWDER Idoc 인터페이스 데이터가 없습니다.</div>
          </div>
        </template>
      </BaseDataTable>
    </v-card>
...
const headers = [
  ...
  { title: 'IDOC 타입', key: 'idocTypId', align: 'center', width: '140px' },
...
function handleExport() {
  ...
  link.setAttribute('download', 'POWDER_IdocHistory_' + new Date().toISOString().slice(0, 10) + '.csv')
